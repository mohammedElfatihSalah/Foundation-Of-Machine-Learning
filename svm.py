...
def svm_loss_vectorized(W, X, y, reg):
  ...
  mask_gt_zero = scores > 0
  loss = torch.sum(scores[mask_gt_zero])
  loss /=num_train
  loss +=reg*torch.sum(W*W)
  #############################################################################
  #                             END OF YOUR CODE                              #
  ...
  #############################################################################
  # Replace "pass" statement with your code
  scores[mask_gt_zero] =1
  scores[scores < 0 ] = 0
  scores[range(num_train), y] = scores.sum(dim=1)*-1
...
def softmax_loss_naive(W, X, y, reg):
  ...
  for i in range(num_train):
    ...
    scores_normalized = torch.exp(scores) / torch.sum(torch.exp(scores))
    loss += -torch.log(scores_normalized[y[i]])

    
    dW += X[i].reshape(-1,1) @ scores_normalized.reshape(1,-1)
    dW[:, y[i]] -= X[i]
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  norm_scores = torch.exp(scores) / torch.exp(scores).sum(dim=1).reshape(-1,1)
  loss += -1*torch.log(norm_scores[range(num_train), y]).sum() 
  loss /= num_train
  loss += reg*torch.sum(W*W)

  dW += X.t().mm(norm_scores)
