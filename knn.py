...
def compute_distances_one_loop(x_train, x_test):
  ...
  test = test.reshape(num_test, test.shape[1]//num_test)

  train = x_train.reshape(1,-1)
  train = train.reshape(num_train, train.shape[1]//num_train)
  for i in range(num_train):
    dists[i] = torch.sum((train[i] - test)**2, dim=1)
...
def compute_distances_no_loops(x_train, x_test):
  ...
  test = test.reshape(num_test, test.shape[0]//num_test)

  train = x_train.reshape(-1,1)
  train = train.reshape(num_train, train.shape[0]//num_train)
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  dists = torch.sum(train**2, dim=1).reshape(-1,1) - 2* train @ test.t() + torch.sum(test **2,dim=1).reshape(1,-1)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
...
def predict_labels(dists, y_train, k=1):
  ...
  predicted_label = y_train[indices]
  predicted_label = torch.mode(predicted_label, dim=0)
  y_pred = predicted_label.values
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return y_pred
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  x_trains_folds = torch.chunk(x_train,num_folds)
  y_trains_folds = torch.chunk(y_train, num_folds)
  print(x_trains_folds[0].shape)
  print(torch.cat(x_trains_folds[0:2],0).shape)
  ...
  for current_k in k_choices:
    ...
    for i in range(num_folds):
      x_valid = x_trains_folds[i]
      y_valid = y_trains_folds[i]
      #print(fold_size)
      
      #print(x_trains_folds[0:0]+x_trains_folds[1:1000])

      X = torch.cat(x_trains_folds[0 : i] + x_trains_folds[(i+1):],0)
      y =  torch.cat(y_trains_folds[0 : i] +y_trains_folds[(i+1):], 0)

      model = KnnClassifier(X,y)
      acu += model.check_accuracy(x_valid,y_valid,current_k, quiet=True)
      #print(acu)
    k_to_accuracies[current_k] = acu / num_folds
  ##############################################################################
  #                            END OF YOUR CODE                                #
  ##############################################################################
...
def knn_get_best_k(k_to_accuracies):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  best_k = sorted(k_to_accuracies.keys(), key=lambda x: (-k_to_accuracies[x],x))[0]
  ##############################################################################
  #                            END OF YOUR CODE                                #
  ##############################################################################
  return best_k
